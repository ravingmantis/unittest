...
Use the ``test_for_error`` function.
...
Use ``cmp_equal(...)`` or ``cmp_identical(...)`` as replacements for ``all.equal(...)`` and ``identical(...)`` respectively:

```{r}
a <- c(1,2,3)
b <- 1:3
ok(cmp_equal(a,b), "a and b are equal")
```
